...
ui <- dashboardPage(skin = "green",
  ...
  dashboardHeader(title = "DCF"
                  ),
  dashboardSidebar(
    sidebarMenu(
      menuItem("Überblick", tabName = "dashboard"), 
      uiOutput("usermanual")
      ...
      tabItem(tabName = "dashboard",
              ...
              fluidRow(
                ...
                fluidRow(column (width = 3, 
                                 conditionalPanel(
                                   condition = "input.modus == 'Monatsende'",
                                 dateInput("date", "Referenzdatum",value = as.Date(as.yearmon(Sys.Date())) - 1)),
                                 conditionalPanel(
                                   condition = "input.modus == 'Simulation'",
                                   dateInput("date", "Referenzdatum",value = Sys.Date()))
                                 
                                 ),
                column(width = 6, fileInput("file1", "Inputfile hochladen", buttonLabel = "Suchen...",placeholder = "")) ),
        
                fluidRow(column(width=4, selectInput("modus", "Berechnungsmodus",c("Monatsende","Simulation"))),
                         column(width = 4,sliderInput(inputId ="Sim_num","Anzahl der Simulationen" ,value = 300, min=300, max=30000))
                ),
                fluidRow( 
                         column(width = 4, sliderInput(inputId ="KI_num","Konfidenzintervall", min=0.95, max=0.999, value = 0.999))),
             
               
                
                fluidRow(
                  column(width=4,actionButton("calculate", "Berechnen")),
                  column(width=4, actionButton("reset", "Werte zurücksetzen")),
                  column(width=4, actionButton("dbSave", "Werte speichern"))
                )
                ),
                column(width = 8,
                       plotOutput("plot1")
            ...
            br()
              ...
              fluidRow(
                ...
                fluidRow(column(width = 4,offset=0.5,
                       p(strong("2. Beteiligungsrisiko - Details"))
...
server <- function(input, output, session) {
  ...
  query_modal <- modalDialog(
    title = "JKU Gastvortrag - 10. November 2025",
    p("Diese Anwendung wurde als Praxisbeispiel für den Gastvortrag an der JKU am 10. November 2025 entwickelt. Mit einem Stern (*) markierte Felder sind für die Berechnung notwendige Inputvariablen.", tags$br(),tags$br(), "Kontaktdaten:", tags$br(), tags$br(),tags$b("Mag. Goran Lovric, LL.M."),tags$br(),tags$a(href="mailto:goran.lovric@rlbooe.com", "goran.lovric@rlbooe.com")),
    easyClose = T,
    size="m",
    footer = tagList(
      actionButton("run", "Bestätigen")
    )
  )
  
  query_modal2 <- modalDialog(
    title = "Benachrichtigung",
    p("Werte sind in die Datenbank gespeichert worden"),
    easyClose = T,
    size="m",
    footer = tagList(
  
      actionButton("ok", "Schließen")
    )
  )
  ...
  # Simulations
    ...
    output$warning3 <- renderValueBox({
      valueBox( 
        ...
        subtitle = "Beteiligungsrisiko - undiversifiziert", 
        icon = icon("percent"),
        color = "aqua"
      )
    })
    ...
    output$Box1_div <- renderValueBox({
      valueBox( 
        ...
        subtitle = "Beteiligungsrisiko - diversifiziert", 
        icon = icon("percent"),
        color = "aqua"
      )
    })
    ...
    output$MC <- renderValueBox({
      valueBox( 
        ...
        subtitle = "Größter Einzelbeitrag - undiversifiziert", 
        icon = icon("fire"),
        color = "red"
      )
    })
    ...
    output$Box2_div <- renderValueBox({
      valueBox( 
        ...
        subtitle = "Größter Einzelbeitrag - diversifiziert", 
        icon = icon("fire"),
        color = "red"
      )
    })
    ...
    output$out <- renderValueBox({
      valueBox( 
        ...
        subtitle = "Value at Risk - undiversifiziert", 
        icon = icon("money"),
        color = "yellow"
      )
    })
    ...
    output$Box3_div <- renderValueBox({
      valueBox( 
        ...
        subtitle = "Value at Risk - diversifziert", 
        icon = icon("money"),
        color = "yellow"
      )
    })
      ...
      output$plot1 <- renderPlot({
        ...
        balance <- data.frame(desc = c("Beteiligung in EUR",sorted_tab$Name[1],
                                       sorted_tab$Name[2], sorted_tab$Name[3], sorted_tab$Name[4],sorted_tab$Name[5],
                                        "Rest", "Exposure exkl. undiv. Beteiligungsrisiko"), amount = c(sum(sorted_tab$MV_ownshare), -(sorted_tab$MV_ownshare[1]-sorted_tab$Value999_ownshare[1]) ,
        ...
        balance$Summary <- ifelse(balance$amount > 0, "Beteiligung",
                               "Beteiligungsrisiko")
        ...
        ggplot(balance, aes(desc, fill = Summary),colour="lightblue") + geom_rect(aes(x = desc,
                                                                                   ...
                                                                                   ymax = start)) +labs(x = "Participation", y="Amount in EUR", title = paste( "Beteiligungsrisiko:",round(part_risk_undiv,2), "Millionen EUR und", round(part_risk_undiv_pct*100,1), "Prozent", sep=" ")) + geom_text(subset = (balance$Summary == "Participation"), aes(id,
  ...
  output$download_cases <-  downloadHandler(
    filename = function() { paste0("Beteiligungsrisiko_",format(Sys.time(), "%Y%m%d_%H%M%S"),".xlsx")},
  ...
  output$usermanual <- renderUI({
      ...
      sidebarUserPanel("",
                       ...
                       subtitle = a(icon("book"), "Benutzerhandbuch", href="",target="_blank")
      )
